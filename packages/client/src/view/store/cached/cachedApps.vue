...
const columns = [
  ...
  { key: 'watch', dataKey: 'watch', title: t('iot.store.watch'), width: 80 },
      ...
      return h(
        ElButtonGroup, [
          ...
          h(ElButton,
            ...
            { disabled: !has_cached, link: true, type: 'primary', icon: 'View', onClick: () => installApp(rowIndex, rowData) },
            { default: () => t('iot.store.install') },
          ),
          h(ElButton,
            // { type: 'danger', icon: 'Delete', onClick: () => handleView(rowIndex, cellData) },
            { disabled: !has_cached, link: true, type: 'primary', icon: 'View', onClick: () => viewApp(rowIndex, rowData) },
            { default: () => t('iot.view') },
          ),
          h(ElButton,
