...
export const useUserStore = defineStore('user', () => {
  ...
  const language = ref(window.localStorage.getItem('language') || 'zh') // added by mohamed hassan to allow store selected language for multilanguage support.
  ...
  const LoginIn = async(loginInfo) => {
    ...
    try {
      ...
      if (res.code === 0) {
        ...
        await router.replace({ name: 'iot.dashboard' })
